...
{
    NSLog(@"Info for current location is: %@",self.locationInfo);
...
{
    NSLog(@"Curren region code is: %@", self.regionCode);
...
- (void)_setup
{
  ...
  locationManager_.delegate = self;
    // ENORM: add 2 lines below to enable location updating
  ...
  [locationManager_ setDesiredAccuracy:kCLLocationAccuracyBest];
    // ENORM: uncomment line below to replace filter 100, in order to test minor location change.
    //[locationManager_ setDistanceFilter:kCLDistanceFilterNone];
  [locationManager_ setDistanceFilter:100];
    // ENORM: add line below to start updating location
...
- (void)_generateLocationInfoForLocation:(CLLocation *)location
{
  ...
  NSString * originalLanguage = [languages objectAtIndex:0];
    NSLog(@"originalLanguage: %@",originalLanguage);
...
// ENORM: replace the locationManager:didUpdateToLocation:fromLocation block below due to deprecated.
...
//            NSLog(@"newLocation info will be generated");
...
//    // ENORM: uncomment ifndef to test location updates
...
//        [self.locationManager stopUpdatingLocation];
//        isUpdatingLocation_ = NO;
//        NSLog(@"updatingLocation is stopped");
//    }
//#endif
//}
