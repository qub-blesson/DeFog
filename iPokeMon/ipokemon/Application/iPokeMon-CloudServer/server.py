...
#
# RESTs (Representational State Transfer)
#
@server.route('/')
def index():
    r = redis.Redis(RADIS_HOST, REDIS_PORT, REDIS_DB)
    r.setnx('global:nextUserid', 0)
    return 'This is Cloud Server Running'

# Remote config
@server.route('/remoteConfig')
def remoteConfig():
    f = open('remoteConfig.xml', 'r')
    configFile = f.read()
    f.close()
    return configFile
...
run(
        ...
        #host     = 'localhost',
        host    =   '0.0.0.0',    # Amazon Cloud Public IP
    port = 8000,               
